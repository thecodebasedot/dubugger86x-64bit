...
static bool resolveApiSetForward(const String & originatingDll, String & forwardDll, String & forwardExport)
{
    ...
    if(!GetSystemDirectoryW(szApiSetDllPath, _countof(szApiSetDllPath)))
        return {};
    wcsncat_s(szApiSetDllPath, MAX_PATH, L"\\downlevel\\", _TRUNCATE);
    wcsncat_s(szApiSetDllPath, MAX_PATH, StringUtils::Utf8ToUtf16(forwardDll).c_str(), _TRUNCATE);
    wcsncat_s(szApiSetDllPath, MAX_PATH, L".dll", _TRUNCATE);
