...
static size_t getSSERegisterOffset(FormatValueType value, size_t elementSize)
{
    ...
    if(buf[1] == 'm' && buf[2] == 'm' && (buf[0] == 'x' || buf[0] == 'y' || (buf[0] == 'z' && AVX512)))  // begins with /[xyz]mm/
    {
        ...
        if(buf[bufptr] == '\0')   // [xy]mm\d{1,2}
            return offsetof(REGDUMP_AVX512, regcontext.ZmmRegisters[0].Low.Low) + index * sizeof(((REGDUMP_AVX512*)0)->regcontext.ZmmRegisters[0]);
        else if(elementSize == 8 && buf[0] == 'x' && buf[bufptr] == 'h' && buf[bufptr + 1] == '\0')   // xmm\d{1,2}h
            return offsetof(REGDUMP_AVX512, regcontext.ZmmRegisters[0].Low.High) + index * sizeof(((REGDUMP_AVX512*)0)->regcontext.ZmmRegisters[0]);
        else if(buf[bufptr] == '[')
        {
            if(buf[bufptr + 1] >= '0' && buf[bufptr + 1] <= '9' && buf[bufptr + 2] == ']' && buf[bufptr + 3] == '\0')   // [xyz]mm\d{1,2}\[\d\]
            {
                ...
                if(buf[0] == 'x' && item >= 0 && item < 16 / elementSize)  // xmm
                    return offsetof(REGDUMP_AVX512, regcontext.ZmmRegisters[0]) + index * sizeof(((REGDUMP_AVX512*)0)->regcontext.ZmmRegisters[0]) + item * elementSize;
