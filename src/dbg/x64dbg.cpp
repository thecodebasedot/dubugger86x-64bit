...
static void registercommands()
{
    ...
    dbgcmdnew("dumpprocess,dumpall", cbDumpProcess, false); //dump all loaded modules
    dbgcmdnew("patchfile,savefile", cbPatchFile, true); //save patched file
    dbgcmdnew("automod,quickreverse", cbAutomod, true); //auto analyze and prepare for patching
    dbgcmdnew("applypatch,importpatches", cbApplyPatchScript, true); //apply patches from script
    ...
    dbgcmdnew("exportdisasm,disasmexport", cbExportDisasm, false); //export disassembly
...
extern "C" DLL_EXPORT const char* _dbg_dbginit(bool blocking)
{
    if(!EngineCheckStructAlignment(UE_STRUCT_TITAN_ENGINE_CONTEXT, sizeof(TITAN_ENGINE_CONTEXT_t)))
        dputs("Warning: Invalid TITAN_ENGINE_CONTEXT_t alignment! Proceeding anyway...");
