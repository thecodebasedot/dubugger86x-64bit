...
extern "C" __declspec(dllexport) HMODULE WINAPI LoadLibraryCheckedW(const wchar_t* lpLibFileName, bool allowFailure)
{
    return LoadLibraryW(lpLibFileName);
}

extern "C" __declspec(dllexport) HMODULE WINAPI LoadLibraryCheckedA(const char* lpLibFileName, bool allowFailure)
{
...
int main(int argc, char* argv[])
{
    ...
    *period = L'\0';
    ...
    if(errormsg != nullptr)
    {
        wprintf(L"BridgeInit failed: %ls\n", errormsg);
        return EXIT_FAILURE;
    }
