...
static HMODULE WINAPI FallbackLoadLibraryCheckedW(const wchar_t* szDll, bool allowFailure)
{
    return LoadLibraryW(szDll);
}

static HMODULE WINAPI FallbackLoadLibraryCheckedA(const char* szDll, bool allowFailure)
{
...
BRIDGE_IMPEXP const wchar_t* BridgeInit(BRIDGE_CONFIG* config)
{
    ...
    if(pLoadLibraryCheckedW == nullptr)
        pLoadLibraryCheckedW = FallbackLoadLibraryCheckedW;
    if(pLoadLibraryCheckedA == nullptr)
        pLoadLibraryCheckedA = FallbackLoadLibraryCheckedA;
